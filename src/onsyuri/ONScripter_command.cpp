...
int ONScripter::ofscopyCommand()
{
    ...
    if (metalfx_presenter && metalfx_presenter->isActive()) {
        // The MetalFX path does not own a renderer framebuffer to read back
        // from; keeping accumulation_surface untouched is the closest
        // no-loss equivalent. Logged for visibility.
        utils::printInfo("[MetalFX] ofscopy skipped: renderer readback unavailable\n");
        return RET_CONTINUE;
    }
#endif
