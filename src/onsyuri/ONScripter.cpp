...
#if defined(MACOSX)
#include <SDL_metal.h>
#include "metalfx/MetalFXPresenter.h"
#endif
...
#if defined(MACOSX)
    const char *game_title = getenv("YOGHOURT_GAME_TITLE");
    if (game_title && game_title[0] != '\0') title = game_title;
#endif
...
void ONScripter::initSDL()
{
    ...
    // SDL window dimensions are logical points on Retina displays. Creating a
    // 1920x1080 game at 1:1 therefore overflows most laptop work areas, so fit
    // only the initial window while keeping the game's native aspect ratio.
    SDL_Rect usable_bounds;
    if (SDL_GetDisplayUsableBounds(0, &usable_bounds) == 0) {
        int fitted_width = screen_device_width;
        int fitted_height = screen_device_height;
        const int max_width = usable_bounds.w * 9 / 10;
        const int max_height = usable_bounds.h * 9 / 10;
        if (fitted_width > max_width) fitted_width = max_width;
        if (fitted_height > max_height) fitted_height = max_height;
        alignAspectRatio(screen_width, screen_height, fitted_width, fitted_height);
        screen_device_width = fitted_width;
        screen_device_height = fitted_height;
    }
#endif
    ...
    // in initSDL leaves the default at nearest, which looks blocky on
    // Retina when games upscale low-resolution frames.
    ...
    if (metalfx_requested) {
        ...
        if (!metal_view) {
            utils::printInfo("[MetalFX] disabled: SDL_Metal_CreateView failed\n");
        }
    }
#endif
...
#if defined(MACOSX)
    if (metalfx_requested && metal_view) {
        metalfx_presenter = new onsyuri_metalfx::Presenter(window, metal_view, screen_width, screen_height);
        if (!metalfx_presenter->isActive()) {
            // The presenter logged the reason; the metal view stays alive so
            // the SDL renderer can keep using it.
        }
    }
#endif
#if defined(MACOSX)
    const char *yoghourt_session = getenv("YOGHOURT_SESSION_ID");
    fprintf(stderr, "[Yoghourt] READY engine=onscripter session=%s\n",
            yoghourt_session && yoghourt_session[0] ? yoghourt_session : "unknown");
    fflush(stderr);
#endif
...
#if defined(MACOSX)
    if (metalfx_presenter && metalfx_presenter->isActive()) {
        // Text layer alpha masks glyph shapes so post-processing skips text.
        metalfx_presenter->setTextLayer(text_info.image_surface);
        if (metalfx_presenter->present(accumulation_surface)) {
            screen_dirty_flag = false;
            return;
        }
    }
    // A failed present already deactivated the presenter; fall through to the
    // SDL path so the current frame is still shown.
#endif
